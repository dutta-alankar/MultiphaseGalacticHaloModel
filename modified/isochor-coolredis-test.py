...
fig = plt.figure()
ax = fig.add_subplot(111)
plt.loglog(b/(mod_isochor.unmodified.Halo.r200*mod_isochor.unmodified.Halo.UNIT_LENGTH/kpc), NOVI, label=r'isothermal with isochoric modification')
#plt.loglog(b/(mod_isochor.unmodified.Halo.r200*mod_isochor.unmodified.Halo.UNIT_LENGTH/kpc), NOVII, label=r'$N_{OVII}$')
plt.loglog(obsData[:,0], obsData[:,1], 'o', label=r'observed data')
plt.grid()
plt.ylim(5e13,2.3e15)
plt.ylabel(r'$N_{\rm OVI} [{\rm cm}^{-2}]$')
plt.xlabel(r'$\rm b/r_{200}$')
plt.title('OVI column density')

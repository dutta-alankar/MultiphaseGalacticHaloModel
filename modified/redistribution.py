...
class Redistribution(ABC):
    ...
    unmodifiedProfile: "unmodified_field"
    TmedVW: float = 3.e5
    ...
    def __post_init__(self: "Redistribution") -> None:
        self.unmodified = self.unmodifiedProfile
        self.sigH   = self.sig
        self.sigW   = self.sig
        self.redshift = self.unmodified.redshift
        self.ionization  = self.unmodified.ionization
    ...
    @abstractmethod
    def _calculate_filling_fraction(self: "Redistribution", xmin: float, indx: int) -> Tuple[float,float]:
        pass
        
    def ProfileGen(self: "Redistribution", radius_: Union[float, list, np.ndarray]) -> Tuple:  #takes in radius_ in kpc, returns Halo density and pressure  in CGS
        mu = Ionization.interpolate_mu
        if not(hasattr(self, "isobaric")):
            raise AttributeError("Error: Missing thermodynamic modification condition: isobaric/isochoric!")
        ...
        tdyn  = np.sqrt(radius**3/(\
               (G*self.UNIT_LENGTH**2*self.UNIT_DENSITY/self.UNIT_VELOCITY**2)\
                   *(self.unmodified.Halo.Mass(radius_)/self.UNIT_MASS))) # code
        tcool = lambda ndens, nH, Temp, met:((1.5+isobaric)*ndens*kB*Temp/(nH*nH*cooling_approx(Temp, met)))\
                /self.UNIT_TIME # ndens in CGS , tcool in code, 4.34 for FM17 LAMDBA norm
        ...
        def _calc(r_val: float) -> None: # This always called for each memeber of radius array
    ...
    def MassGen(self: "Redistribution", radius_: Union[float, list, np.ndarray], isobaric: int) -> Tuple[float, float]: # Takes in r in kpc, returns Halo gas mass of each component in CGS
    ...
    def PlotDistributionGen(self: "Redistribution", radius: float, figure: Optional[plt.figure] = None) -> None: # Takes in radius in kpc (one value at any call)
        _call_profile = True # Check if ProfileGen needs to be called
        _save = True
        ...
        plt.vlines(np.log10(self.TmedVH), 1e-3, 2.1, colors='tab:red', linestyles=':', label=r'$T_{med,V}^{(h)}$', 
        ...
        if (self.redisType is not None and _save):
            save_dict = {"T_cutoff": Tcutoff, "T_hot_M": self.TmedVH*np.exp(-self.sigH**2/2), "T_med_VW": self.TmedVW, "T_hot_u": self.TmedVu*np.exp(-self.sig**2/2),
                         "TempDist": self.TempDist, "Hot_mod": np.piecewise(gvh,[self.TempDist>=Tcutoff,],[lambda val:val, lambda val:0]),
                         "gv_h": gvh, "gv_w": gvw, "sig_u": self.sig, "sig_H": self.sigH, "sig_W": self.sigW, 
                         "cutoff": self.cutoff, "ionization": self.ionization, "radius": radius, }
            os.system("mkdir -p ./figures")
            with open(f'figures/{self.unmodified.unmod_type}_{self.redisType}_{self.ionization}_distrib-r={radius:.1f}kpc.pickle', 'wb') as f:
                pickle.dump(save_dict, f)
        
        if figure is None:   
            unmod = "isothermal" if self.unmodified.unmod_type=="isoth" else "isentropic"
            ...
            plt.grid()
            plt.title(r'$r = $%.1f kpc [%s with isochoric modification] (%s)'%(radius, unmod, self.ionization), size=28)
            plt.ylim(1e-3, 2.1)
            plt.xlim(5, 7)

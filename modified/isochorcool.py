...
class IsochorCoolRedistribution:
    ...
    def ProfileGen(self, radius_):  #takes in radius_ in kpc, returns Halo density and pressure  in CGS
        ...
        # if (len(unmod_nH[unmod_nH<1e-6])>0):
        #     print('unmod')
        #     print(unmod_nH[unmod_nH<1e-6])
        #     print(radius_[unmod_nH<1e-6])
        # print(unmod_nH)
        ...
        def _calc(r_val): # This always called for each memeber of radius array
            indx = np.argwhere(np.isclose(radius, r_val))[0,0]
    ...
    def PlotDistributionGen(self, radius_): #takes in radius_ in kpc
        ...
        radius = np.copy(radius_)*kpc/IsochorCoolRedistribution.UNIT_LENGTH
        unmod_rho, unmod_prsTh, _, _, unmod_prsTot, unmod_nH, unmod_mu = self.unmodified.ProfileGen(radius_) #CGS
        ...
        for indx, r_val in enumerate(radius):
            ...
            plt.grid()
            plt.title(r'$r = $%.1f kpc [isothermal with isochoric modification] (%s)'%(radius_, self.ionization), size=28)
            plt.ylim(1e-3, 2.1)
            plt.xlim(5, 7)

...
from misc.constants import kpc, MSun, cm, K, kB
from unmodified import isent
import numpy as np
...
radius = np.linspace(
    ...
    Model.rCGM * Model.UNIT_LENGTH / kpc,
    Npts,
)  # kpc
...
n = Model.ndens
T = Model.Temperature
Mcor = integrate.cumtrapz(4 * np.pi * (radius * kpc) ** 2 * rho, radius * kpc) / MSun
# Mcor = rho*(4/3)*pi*radius**3*Model.UNIT_MASS/MSun
met = Model.metallicity
ZmetAvg = integrate.cumtrapz(
    4 * np.pi * (radius * kpc) ** 2 * rho * met, radius * kpc
) / (Mcor * MSun)
...
plt.semilogy(
    radius, 1.3e-5 * (radius / Model.rCGM) ** (-0.93) * cm ** (-3), ls=":", label="Fit"
)
plt.grid()
plt.legend(loc="best")
...
plt.savefig("figures/nH-isent.png")
plt.cla()
...
ax.yaxis.set_ticks_position("both")
plt.savefig("figures/mu-isent.png")
plt.cla()
...
plt.savefig("figures/temp-isent.png")
plt.cla()
...
plt.semilogy(
    radius,
    22.1 * (radius / Model.rCGM) ** (-1.35) * K * cm ** (-3),
    ls=":",
    label="Fit",
)
plt.grid()
...
# ax.set_xlim(0,250)
ax.set_ylim(1, 1e4)
plt.savefig("figures/pres-isent.png")
plt.cla()
...
# ax.set_xlim(0,250)
ax.set_ylim(0, 1)
plt.savefig("figures/pres-all-isent.png")
plt.cla()

fig = plt.figure()
ax = fig.add_subplot(111)
plt.plot(radius, np.sqrt(4 * np.pi * PNTh) * 1e9)
plt.plot(
    [
        Model.Halo.r0 * Model.Halo.UNIT_LENGTH / kpc,
        Model.rCGM * Model.UNIT_LENGTH / kpc,
...
plt.xlabel("radius [kpc]")
ax.yaxis.set_ticks_position("both")
plt.savefig("figures/magB-isent.png")
plt.cla()
...
plt.vlines(
    Model.Halo.r200 * Model.Halo.UNIT_LENGTH / kpc,
    1e2,
    1e18,
    colors="black",
    linestyles="--",
...
plt.savefig("figures/mass-isent.png")
plt.cla()
...
plt.plot(radius, met)
plt.vlines(
    Model.rZ * Model.UNIT_LENGTH / kpc,
    0,
    2,
    colors="black",
    linestyles="--",
...
ax.yaxis.set_ticks_position("both")
ax.set_ylim(0.2, 1.2)
plt.savefig("figures/met-isent.png")
plt.cla()

...
def plot_map(mod: str, ionization: str, map_type: str) -> None:
    print(mod, ionization, map_type)
    ...
    if mod == "isochor":
        title = r"$\gamma $ = [1 polytrope - 5/3 polytrope] (IC)"
    else:
        title = r"$\gamma $ = [1 polytrope - 5/3 polytrope] (IB)"
    cmap = "jet" 
    ...
    with open(
        f"figures/map_{map_type}_isoth_{mod}_{ionization}.pickle", "rb"
    ) as data_file:
        data = pickle.load(data_file)
        # print(list(data.keys()))
        l = data["l"]
        b = data["b"]
        map_val_iso = data["map"]
        disk_val_iso = data["disk"]

    with open(
        f"figures/map_{map_type}_isent_{mod}_{ionization}.pickle", "rb"
    ) as data_file:
        data = pickle.load(data_file)
        # print(list(data.keys()))
        l = data["l"]
        b = data["b"]
        map_val_ise = data["map"]
        disk_val_ise = data["disk"]
    
    diff_ = (map_val_iso  - map_val_ise ) #+ (disk_val_iso - disk_val_ise)#isothermal - isentropic
    ...
    cs = ax.contourf(
        np.deg2rad(l),
        np.deg2rad(b),
        (diff_) * (1e3 if map_type == "emission" else 1.0),
        levels=levels,
        cmap=cmap,
        vmin=vmin,
        vmax=vmax,
    ...
    plt.savefig(
        f"figures/map_diff_{map_type}_{mod}_{ionization}.png",
        transparent=False,
        bbox_inches="tight",
    )
    plt.close()
    ...
    plt.savefig(
        f"figures/map_diff_moll_{map_type}_{mod}_{ionization}.png",
        transparent=False,
        bbox_inches="tight",
    )
    plt.close()
...
if __name__ == "__main__":
    ...
    for condition in product(mod, ionization, map_type):
        plot_map(*condition)

...
def calc_SB(
    Emin: float, Emax: float, energy: np.ndarray, sed: np.ndarray
) -> None:
    pc = 3.0856775807e18
    kpc = 1e3 * pc
    ...
    print(
        f"SB ({Emin:.1f}-{Emax:.1f} keV): {(SB):.2e} erg cm^-2 s^-1 deg^-2"
    )
...
def spectrum(
    ...
    with open(f"figures/SB_{unmod}_{mod}_{ionization}.pickle", "rb") as data_file:
    ...
    SB_tot = sb_hot + sb_warm
    
    Emin = 0.3  # keV
    Emax = [0.6, 2.0]  # keV
    #_ = list(map(lambda till: calc_SB(Emin, till, energy, SB_tot), Emax))
...
if __name__ == "__main__":
    ...
    plt.ylim(ymin=1e-17, ymax=1e-9)
    plt.ylabel(r"Surface Brightness [$erg\ s^{-1}\ cm^{-2}\ deg^{-2}\ keV^{-1}$]", size=28, color="black")

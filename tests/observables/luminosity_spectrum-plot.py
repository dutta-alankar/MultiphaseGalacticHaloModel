...
def calc_SB(Emin: float, Emax: float, 
            energy: np.ndarray, sed: np.ndarray, 
            rCGM: float) -> None:
    pc     = 3.0856775807e18
    kpc    = 1e3*pc
    ...
    print(f"SB ({Emin:.1f}-{Emax:.1f} keV): {(SB/(180/np.pi)):.2e} erg cm^-2 s^-1 deg^-2")

def spectrum(unmod: str, mod: str, ionization: str, fig: plt.figure) -> matplotlib.lines.Line2D: 
    print(unmod, mod, ionization) 
    
    with open(f'figures/spectrum_{unmod}_{mod}_{ionization}.pickle', 'rb') as data_file:
    ...
    plotted_line = ax.loglog(energy, luminosity, label=f"{unmod_label} with {mod_label} re-distribution ({ionization})")
...
if __name__ == "__main__":
    ...
    curves = []
    for condition in product(unmod, [mod[0],], [ionization[0],]):
    ...
    plt.legend(loc='lower left', prop={'size': 20}, framealpha=0.3, shadow=False, fancybox=True)
    plt.savefig(f'figures/spectrum_{unmod[0]+unmod[1]}_{mod[0]}_{ionization[0]}.png', transparent=False)
    # plt.show()
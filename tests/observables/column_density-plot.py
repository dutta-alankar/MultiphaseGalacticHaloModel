...
def plot_column_density(unmod, mod, ion):
    ...
    for ionization in states:
        with open(f'figures/N_{ion}_{unmod}_{mod}_{ionization}.pickle','rb') as data_file:
            data = pickle.load(data_file)
            impact = data['impact']
            column_density = data[f'N_{ion}']
            rCGM = data['rCGM']
            
            extra_label = ("Isothermal" if unmod=="isoth" else "Isentropic") + (" (IC)" if mod=="isochor" else "(IB)")
            if ionization=="PIE":
                pl1 = plt.loglog(np.array(impact)/rCGM, column_density, 
                                 label=r"$N_{%s}$ "%(ion,) + extra_label, 
                                 linestyle='-')
            else:
                plt.loglog(np.array(impact)/rCGM, column_density,
                           linestyle='--', color=pl1[0].get_color())
...
if __name__ == "__main__":
    ...
    coldens_min, coldens_max, coldens_detect, e_coldens_detect = observation.col_density_gen(element = element)
    ...
    plt.errorbar(impact_select_detect/rvir_select_detect, 10.**coldens_detect, yerr=yerr, 
                 fmt='o', color='black', label=r'$\rm N_{%s, obs}$'%element, markersize=10)
    plt.plot(impact_select_min/rvir_select_min, 10.**coldens_min, '^', color='black', markersize=10)
    plt.plot(impact_select_max/rvir_select_max, 10.**coldens_max, 'v', color='black', markersize=10)
    
    plt.legend()  
    plt.xlabel(r"Impact parameter [$r_{vir}$]")
    plt.ylabel(r"Column density ($cm^{-2}$)")
    plt.xlim(xmin = 0.05, xmax=1.1)
    plt.ylim(ymin=10**11.7, ymax=10.**15.3)

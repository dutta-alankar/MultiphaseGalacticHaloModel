...
vol_pdf, bin_edges = np.histogram(np.log10(file["/Temperature"]), bins=300, density=True, weights=np.array(file["/Volume"]))
centers = 0.5*(bin_edges[1:]+bin_edges[:-1])

plt.semilogy(centers, vol_pdf, label="volume")

mass_pdf, bin_edges = np.histogram(np.log10(file["/Temperature"]), bins=300, density=True, 
        weights=np.array(file["/NumberDensity"])*np.array(file["/Volume"]))
centers = 0.5*(bin_edges[1:]+bin_edges[:-1])
plt.semilogy(centers, mass_pdf, label="mass")

emm_pdf, bin_edges = np.histogram(np.log10(file["/Temperature"]), bins=300, density=True, 
                                  weights=-np.array(file["/nH"])**2*np.array(file["/Lambda"])*np.array(file["/Volume"]) )
centers = 0.5*(bin_edges[1:]+bin_edges[:-1])
...
np.savetxt('tng50-pdf-data.txt', np.vstack((centers, vol_pdf, mass_pdf, emm_pdf)).T, 

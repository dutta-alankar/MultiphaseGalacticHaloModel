...
np.save( 'parameters.npy', np.array([f_Vh, f_Vw, f_Vc, x_h, x_w, x_c, sig_h, sig_w, sig_c, T_u]) )
...
beta_h = del_h*(x_h + sig_u**2/2 + del_h*sig_h**2/2)/(x_h + sig_u**2/2 + sig_h**2*(del_h-0.5) )
beta_w = del_w*(x_w + sig_u**2/2 + del_w*sig_w**2/2)/(x_w + sig_w**2/2 + sig_w**2*(del_w-0.5) )
beta_c = del_c*(x_c + sig_u**2/2 + del_c*sig_c**2/2)/(x_c + sig_c**2/2 + sig_c**2*(del_c-0.5) )
...
Den = f_Mh + f_Mw + f_Mc
f_Mh = f_Mh/Den; f_Mw = f_Mw/Den; f_Mc = f_Mc/Den
...
M_pdf_m = f_Vh*np.exp((del_h-1)*(x_h+(del_h-1)*sig_h**2/2+sig_u**2/2))*N_pdf(x,x_h-(1-del_h)*sig_h*sig_h,sig_h)
M_pdf_m += f_Vw*np.exp((del_w-1)*(x_w+(del_w-1)*sig_w**2/2+sig_u**2/2))*N_pdf(x,x_w-(1-del_w)*sig_w*sig_w,sig_w)
M_pdf_m += f_Vc*np.exp((del_c-1)*(x_c+(del_c-1)*sig_c**2/2+sig_u**2/2))*N_pdf(x,x_c-(1-del_c)*sig_c*sig_c,sig_c)
M_pdf_m *= (rho_av_u/rho_av)
# plt.semilogy(np.log10(T),M_pdf_m, '--',color='tab:orange',label='mass PDF')

#using exact luminosity function
hot_lum = np.exp((del_h-1)*sig_u**2)*f_Vh*cooling_approx(np.exp(x)*T_u, metallicity)*np.exp(2*(del_h-1)*(x_h+(del_h-1)*sig_h*sig_h))*N_pdf(x,x_h+2*(del_h-1)*sig_h*sig_h,sig_h)
L_pdf_m = hot_lum
warm_lum = np.exp((del_w-1)*sig_u**2)*f_Vw*cooling_approx(np.exp(x)*T_u, metallicity)*np.exp(2*(del_w-1)*(x_w+(del_w-1)*sig_w*sig_w))*N_pdf(x,x_w+2*(del_w-1)*sig_w*sig_w,sig_w)
L_pdf_m += warm_lum
cold_lum = np.exp((del_c-1)*sig_u**2)*f_Vc*cooling_approx(np.exp(x)*T_u, metallicity)*np.exp(2*(del_c-1)*(x_c+(del_c-1)*sig_c*sig_c))*N_pdf(x,x_c+2*(del_c-1)*sig_c*sig_c,sig_c)
L_pdf_m += cold_lum
L_pdf_m /= np.trapz(L_pdf_m,x)

# plt.semilogy(np.log10(T),L_pdf_m, '--', color='tab:green', label='luminosity PDF')

f_Lh = np.trapz(hot_lum,x); f_Lw = np.trapz(warm_lum,x); f_Lc = np.trapz(cold_lum,x)
Den = f_Lh+f_Lw+f_Lc; f_Lh = f_Lh/Den; f_Lw = f_Lw/Den; f_Lc = f_Lc/Den

np.save('3PhasePdf-LogTu=%.1fK.npy'%np.log10(T_u), np.vstack((np.log10(T),V_pdf,M_pdf_m,L_pdf_m)).T )

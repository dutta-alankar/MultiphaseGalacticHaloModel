...
prs = interpolate.interp1d(radius, prsTh, fill_value="extrapolate")  # CGS
density = interpolate.interp1d(radius, rho, fill_value="extrapolate")  # CGS
metallicity = interpolate.interp1d(
    radius, unmodified.metallicity, fill_value="extrapolate"
)
    ...
    for num, cloud in enumerate(clouds):
        ...
        if distance < cloud_size:
            ...
            cloud_intersect.append(
                [num, 2 * np.sqrt(cloud_size**2 - distance**2), nIonCloud]
            )
...
clouds = [
    {
        ...
        "fIon": pow(
            10.0,
            fIon(
                nH=rho_cloud[i] * Xp(met_cloud[i]) / mH,
                temperature=TmedVW,
...
np.savez(
    f"randomSight_e.{element}_i.{ion}.npz",
    impact=impact,
    col_dens=col_dens,
    rCGM=rCGM,
)

...
def log_prior(params):
    def lnnormal(x, mu, sig):
        return -np.log(np.sqrt(2 * np.pi) * sig) - (x - mu) * (x - mu) / (
            2.0 * sig * sig
        )
    ...
    x_c = np.log10(1.5e4)
    lp = np.sum(
        lnnormal(params[0], x_h, 0.1)
        + lnnormal(params[1], x_w, 0.2)
        + lnnormal(params[2], x_c, 0.1)
    ...
    10.0 ** params[0]  # 10.**6.4

    sig_h = params[5]  # 0.52
    sig_w = params[6]  # 1.4
    params[7]  # 0.4

    10.0 ** params[1]  # 10**5.5
    10.0 ** params[2]  # 1.4e4
...
flat_samples = sampler.get_chain(
    discard=int(5 * np.ceil(np.max(tau))), thin=int(np.ceil(np.max(tau) / 2)), flat=True
)

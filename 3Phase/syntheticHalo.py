...
def shoot_lines(b, r_vir, point, nlines=20, res=100):
    ...
    for line in range(nlines):
        direction = [
            -point[0] + b * sin_theta[line] * cos_phi[line],
        ...
        along_los = np.array(
            [distance * direction[0], distance * direction[1], distance * direction[2]]
        )
...
x, y, z = (
    r_vir * np.sin(theta) * np.cos(phi),
    r_vir * np.sin(theta) * np.sin(phi),
    r_vir * np.cos(theta),
)

...
def nIon_avg(element, ion, mode):
    ...
    fIon = 10.0 ** np.array(
        [
            fIon(PbykB / T_val, T_val, metallicity, redshift, element, ion, mode)
            for T_val in Temperature
        ]
    )
    nIon_avg = np.trapz(
        abundance(element)
...
ne = np.array(
    [
        num_dens(PbykB / T_val, T_val, metallicity, redshift, mode, "electron")
        for T_val in Temperature
    ]
)
ni = np.array(
    [
        num_dens(PbykB / T_val, T_val, metallicity, redshift, mode, "ion")
        for T_val in Temperature
    ]
)
...
def IonColumn(element, ion, name, ylim=None, fignum=None, color=None):
    ...
    (
        ...
        coldens_detect,
        e_coldens_detect,
    ) = observedColDens().col_densGen(element=name)
...
ion = 6
name = "O VI"
IonColumn(element, ion, name, ylim=(10**14.0, 10.0**15.3))

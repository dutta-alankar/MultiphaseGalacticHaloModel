...
class HaloModel:
    ...
    def __post_init__(self: "HaloModel"):
        ...
        self.r200 = (3 * self.M200 / (800 * np.pi * (dcrit0 / self.UNIT_DENSITY))) ** (
            1 / 3.0
        )
        self.rs = self.r200 / self.C

    def Mass(
        self: "HaloModel", r: Union[float, int, list, np.ndarray]
    ) -> np.ndarray:  # takes in r in kpc, returns Halo Mass in CGS
        r_ = np.array(r) * kpc / self.UNIT_LENGTH

        def f(x):
            return np.log(1 + x) - x / (1 + x)

        def MHalo(rp):
            return self.M200 * f(rp / self.rs) / f(self.C)  # NFW

        def MDisk(rp):
            return (
                0.025 * (1 - np.exp(-2.64 * (rp / (kpc / self.UNIT_LENGTH)) ** 1.15))
                + 0.142
                * (
                    1
                    - (1 + (rp / (kpc / self.UNIT_LENGTH)) ** 1.5)
                    * np.exp(-((rp / (kpc / self.UNIT_LENGTH)) ** 1.5))
                )
                + 0.833 * (1 - (1 + rp / self.rd) * np.exp(-rp / self.rd))
            ) * self.Mb

        return (
            self.MBH + MHalo(r_) + (MDisk(r_) if self._withDisk else 0.0)
        ) * self.UNIT_MASS

    def Phi(
        self: "HaloModel", r: Union[float, int, list, np.ndarray]
    ) -> np.ndarray:  # takes in r in kpc, returns Halo Potential in CGS
        r_ = np.array(r) * kpc / self.UNIT_LENGTH
        ...
        def _phi(rpp):
            return (
                ...
                * integrate.quad(
                    lambda rp: self.Mass(rp) / self.UNIT_MASS / rp**2, self.r0, rpp
                )[0]
            )

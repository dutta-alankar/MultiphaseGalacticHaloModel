...
class ion_column(ColumnDensity):
    ...
    def _additional_fields(self: "ion_column", indx: int, r_val: float) -> None:
        ...
        hotInt  = np.trapz( (self.mu_hot[indx,:]*self.prs_hot(r_val)/(kB*self.Temp))*self.fIonHot[indx,:]*gvhT, xh) # Global density sensitive (1-self.fvw(r_val))*
        warmInt = np.trapz( (self.mu_warm[indx,:]*self.prs_warm(r_val)/(kB*self.Temp))*self.fIonWarm[indx,:]*gvwT, xw) # self.fvw(r_val)*
    ...
    def gen_column(self: "ion_column", b_: Union[list, np.ndarray, float, int], 
                   element: Union[int, str, AtmElement], ion: Optional[int] = None) -> np.ndarray:

...
def luminosity_spectrum_gen(redisProf: modified_field) -> np.ndarray: 
    ...
    if (_do_unmodified and not(_use_cloudy)):
        ...
        return np.vstack((EmissionSpectrum._energy, luminosityTot/EmissionSpectrum._energy)).T
    ...
    for indx, r_val in enumerate(radius) :
        ...
        PvhT = np.exp(-xh**2/(2*redisProf.sigH**2))/(redisProf.sigH*np.sqrt(2*np.pi))
        xw = np.log(Temp/redisProf.TmedVW)
        gvwT = fvw[indx]*np.exp(-xw**2/(2*redisProf.sigW**2))/(redisProf.sigW*np.sqrt(2*np.pi))

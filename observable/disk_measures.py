...
class MeasuresDisk(maps.MapInit, ABC):
    ...
    def make_map(self: "MeasuresDisk", 
        ...
        if isinstance(l, np.ndarray):          
            if showProgress: 
                ...
                for i in range(field.shape[0]):
                    for j in range(field.shape[1]):
                        ...
                        radius, phi, theta = transform.toGalC(np.array(l)[i,j], np.array(b)[i,j], LOSsample)
                        ...
                        val = np.trapz( np.nan_to_num(self._field_func(radius, height)) , LOSsample)
                            
                        field[i,j] =  val # CGS unit * kpc
                        if (i==0 and j==0): progBar = ProgressBar()
                        progBar.progress(i*field.shape[1]+j+1, 
                                         field.shape[0]*field.shape[1])
                progBar.end()
            else:
                def _calc(tup):
                    l_val, b_val, _integrateTill = tup
                    LOSsample = np.logspace(np.log10(1e-3*_integrateTill), np.log10(_integrateTill), 100) 
                    radius, phi, theta = transform.toGalC(l_val, b_val, LOSsample)
                    height = np.abs(radius*np.cos(np.deg2rad(theta)))
                    radius = np.abs(radius*np.sin(np.deg2rad(theta))) # np.cos(np.deg2rad(phi))) #along disk
                    field = np.trapz( np.nan_to_num(self._field_func(radius, height)), LOSsample)
                    return field # CGS kpc
                    
                tup = (*zip(np.array(l).flatten(), np.array(b).flatten(), self.integrateTill.flatten()),)
            ...
            height = np.abs(radius*np.cos(np.deg2rad(theta)))
            radius = np.abs(radius*np.sin(np.deg2rad(theta))) #np.cos(np.deg2rad(phi))) #along disk
            field = np.trapz( np.nan_to_num(self._field_func(radius, height)), LOSsample) # CGS kpc
...
class Disk_profile:
    ...
    def __post_init__(self: "Disk_profile") -> None:             
        ...
        self.nH: Callable = lambda R, z: self.nH0 * np.exp(-(R/self.R0 + np.fabs(z)/self.z0))
        num_dens = Ionization.interpolate_num_dens
        self.ne0: float = num_dens(self.nH0, self.TDisk, self.metallicity, self.redshift, self.mode, 'electron')
        self.ni0: float = num_dens(self.nH0, self.TDisk, self.metallicity, self.redshift, self.mode, 'ion')       

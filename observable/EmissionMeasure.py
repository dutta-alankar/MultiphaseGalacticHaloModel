...
class EmissionMeasure(maps.MapInit):
    ...
    def ni_gen(self, distance):
        ...
        if not(self.genProf):
            ...
            print("Doing one time profile calculation", flush=True)
            radius_ = np.logspace(np.log10(9.0), np.log10(rend),20) #kpc
            nhot_local, nwarm_local, nhot_global, nwarm_global, fvw, fmw, prs_hot, prs_warm, Tcut = self.redisProf.ProfileGen(radius_)
            self.genProf = True
            np.save('mod-debug.npy', np.vstack((radius_, nhot_local, nwarm_local, prs_hot/(nhot_local*kB), prs_warm/(nwarm_local*kB) )).T ) 
            # print(nhot_local, nwarm_local)
        ...
        THotM  = interp1d( self.redisProf.radius, (self.redisProf.prs_hot/(self.redisProf.nhot_local*kB)), fill_value='extrapolate')
        # THotM  = interp1d( self.redisProf.radius, (self.redisProf.prs_hot/self.redisProf.rhohot_local)*(kB/(self.redisProf.mu_hot*mp)), fill_value='extrapolate')
        # TWarmM = interp1d( self.redisProf.radius, (self.redisProf.prs_warm/self.redisProf.rhowarm_local)*(kB/(self.redisProf.mu_warm*mp)), fill_value='extrapolate')
        ...
        def _calc(r_val):    
            ...
            # print(nHhot-nHhot_t, nHwarm-nHwarm_t)
            tup = (*zip( nHhot, Temp, 
                         metallicity(r_val)*np.ones(Temp.shape[0]), 
                         redshift*np.ones(Temp.shape[0]),
                         [mode,]*Temp.shape[0], ['ion',]*Temp.shape[0] ),)
            
            niHot = np.array( (*map(num_dens, tup),) ).reshape(Temp.shape)
            ...
            niWarm = np.array( (*map(num_dens, tup),) ).reshape(Temp.shape)
            
            '''
            niHot  = np.array([num_dens(     nHhot[i], 
                                             Temp[i], 
                                             metallicity(r_val), redshift, 
                                             mode=mode, part_type='electron' ) for i,xhp in enumerate(xh)])
            
            niWarm = np.array([num_dens(     nHwarm[i], 
                                             Temp[i], 
                                             metallicity(r_val), redshift, 
                                             mode=mode, part_type='electron' ) for i,xwp in enumerate(xw)])
            '''
            hotInt  = (1-fvw(r_val))*np.trapz( niHot*gvhT, xh ) #global density sensitive, extra filling factor for global 
        
            warmInt = fvw(r_val)*np.trapz( niWarm*gvwT, xw )
            
            ni = hotInt + warmInt
            return ni
        
        ni = np.array((*map(_calc, distance),)) 
        # ni = np.piecewise(ni, [ni>=1e-6,], [lambda x:x, lambda x:0.] )
        return ni
    ...
    def generate(self, l, b, showProgress=True):
        ...
        print('Generating free electron and ion profile')
        ne_prof = self.ne_gen(distance)
        ni_prof = self.ni_gen(distance)
        print('Complete!')
        print('ini: rad: ', distance)
        print('ini: ne:  ', ne_prof)
        print('ini: ni:  ', ni_prof)
        np.save('test-debug-ne.npy', np.vstack((distance, ne_prof)).T )
        np.save('test-debug-ni.npy', np.vstack((distance, ni_prof)).T )
        ...
        if isinstance(l, np.ndarray):          
            if showProgress: 
                ...
                for i in range(self.EM.shape[0]):
                    for j in range(self.EM.shape[1]):
                        ...
                        # print('ni: ', np.nan_to_num(10.**ni_prof(np.log10(distance))) )
                        em = np.trapz( np.nan_to_num(10.**ne_prof(np.log10(distance)))*np.nan_to_num(10.**ni_prof(np.log10(distance))) , LOSsample)
                        # print('em = ', em)
            ...
            radius = np.abs(radius*np.sin(np.deg2rad(theta))) #np.cos(np.deg2rad(phi))) #along disk
            distance = np.sqrt(radius**2 + height**2)
            self.EM = np.trapz( np.nan_to_num(10.**ne_prof(np.log10(distance))) * np.nan_to_num(10.**ni_prof(np.log10(distance))), LOSsample) #cm^-6 kpc

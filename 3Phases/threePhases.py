...
V_pdf_c = f_Vc * N_pdf(x, x_c, sig_c)
V_pdf_w = f_Vw * N_pdf(x, x_w, sig_w)
V_pdf_h = f_Vh * N_pdf(x, x_h, sig_h)
...
with open('data_param.pickle', 'wb') as f:
    pickle.dump(data_param, f)
...
print("mass fractions: cold, warm, hot:", f_Mc, f_Mw, f_Mh)
...
M_pdf_c *= rho_av_u / rho_av
M_pdf_w *= rho_av_u / rho_av
M_pdf_h *= rho_av_u / rho_av
M_pdf_m /= np.trapz(M_pdf_m,x)
#print(np.trapz(M_pdf_m,x))
...
hot_lum /= np.trapz(L_pdf_m, x)
warm_lum /= np.trapz(L_pdf_m, x)
cold_lum /= np.trapz(L_pdf_m, x)
L_pdf_m /= np.trapz(L_pdf_m, x)
#print(np.trapz(L_pdf_m, x))

f_Lh = np.trapz(hot_lum, x)
f_Lw = np.trapz(warm_lum, x)
f_Lc = np.trapz(cold_lum, x)
print(f_Lc,f_Lw,f_Lh)

rho = f_Lh + f_Lw + f_Lc
f_Lh = f_Lh / rho
f_Lw = f_Lw / rho
f_Lc = f_Lc / rho
print("luminosity fractions: cold, warm, hot:", f_Lc, f_Lw, f_Lh)

#plt.loglog(T,cooling_approx(T,metallicity))
#plt.show()

os.system("mkdir -p ./figures")
np.save("./figures/3PhasesPdf.npy", np.vstack((np.log10(T), V_pdf, M_pdf_m, L_pdf_m, V_pdf_c,V_pdf_w,V_pdf_h,M_pdf_c,M_pdf_w,M_pdf_h,cold_lum,warm_lum,hot_lum)).T)

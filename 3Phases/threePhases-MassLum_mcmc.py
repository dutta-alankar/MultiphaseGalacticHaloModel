...
if __name__ == "__main__":
    ...
    for i in range(2):
        pdf = np.log10(
            np.piecewise(
                ...
                [lambda x: x, lambda x: 0.1 * np.min(tng50[:, i+2][tng50[:, i+2] > 0.0])],
            )
            / np.log(10)
        )    
        pdf_data[i,:] = pdf[clip]
    ...
    params = np.array([del_h, del_w, del_c, c_h, c_w, c_c])
